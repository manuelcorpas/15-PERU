...
def main():
    ...
    plink_executable_path = 'plink'
    plink_format_output   = 'ANALYSIS/15-DI-STAT/common_variants_ibd_clean_pruned' 
    bed_file_path         = plink_format_output 

    #subprocess.run([plink_executable_path, '--vcf', vcf_file_path, '--make-bed', '--double-id', '--out', plink_format_output])
    
    populations = read_population_file(population_file_path)
    # Filter out populations with less than 12 individuals
    populations = {pop: inds for pop, inds in populations.items() if len(inds) >= 12}

    with pyplink.PyPlink(bed_file_path) as plink:
        fam_df = plink.get_fam()
        print("Column names in FAM file:", fam_df.columns)  # Print the column names

        # Use the correct column for individual IDs
        # Replace 'YourColumnLabel' with the actual column label for individual IDs
        fam_ids = fam_df['iid'].tolist()
    genotypes_by_pop = read_plink_files(bed_file_path, populations, fam_ids)
    allele_freqs_by_pop = calculate_allele_frequencies(genotypes_by_pop)
    top_snps_by_pop = prioritize_snps(allele_freqs_by_pop)
    for pop in top_snps_by_pop:
        for snp in top_snps_by_pop[pop]:
            print (pop,snp,top_snps_by_pop[pop][snp],sep='\t')
    #plot_top_snps(top_snps_by_pop)
...
plink --vcf path_to_vcf_file.vcf --make-bed --out output_prefix
